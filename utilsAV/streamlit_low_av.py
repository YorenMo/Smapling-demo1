# -*- coding: utf-8 -*-
"""
Created on Tue Oct 11 15:27:46 2022
...
def evaluate_results(t, model, number_initial_points= 10, num_plot_markers=20, num_interval_points=1,directory='.', task='Classification'): #
    '''
    ...
    HighAV_sample_index_lists = HighAV_Sample(X, val_shapley, plot_points, directory, t)
    ...
    LowAV_sample_index_lists = LowAV_Sample(X, val_shapley, plot_points, t)
    ...
    sio.savemat(directory+'/LowAV_scores.mat', {'LowAV_scores':LowAV_scores} )
...
def HighAV_Sample(X, V, num_list, directory, t):

    V = V - np.min(V)
   
    M = [gaussian_membership(X, X[i], t) for i in range(len(X))]
...
def LowAV_Sample(X, V, num_list, t):
...
def gaussian_membership(X, mu, t):
    '''
    Parameters
    ----------
