...
def evaluate_results(basemodel, number_initial_points, num_plot_markers=20, num_interval_points=1,directory='.', task='Classification'): #
    ...
    RH_scores = [evaluate_performance(basemodel, sample_index.copy(), directory) for sample_index in RH_sample_index_lists]
    # remove low value points
    RL_sample_index_lists = [ low_val_index[i:] for i in plot_points]
    RL_scores = [evaluate_performance(basemodel, sample_index.copy(), directory) for sample_index in RL_sample_index_lists]
    # add high value points
    AH_sample_index_lists = [ high_val_index[:i + number_initial_points] for i in plot_points]
    AH_scores = [evaluate_performance(basemodel, sample_index.copy(), directory) for sample_index in AH_sample_index_lists]
    # add low value points
    AL_sample_index_lists = [ low_val_index[:i + number_initial_points] for i in plot_points]
    AL_scores = [evaluate_performance(basemodel, sample_index.copy(), directory) for sample_index in AL_sample_index_lists]
